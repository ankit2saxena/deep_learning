...
from keras.layers import AveragePooling1D, AveragePooling2D, Dropout
...
def LSTMResNet34Model(lr, m, d):
    ...
    #print(X_full.shape)
    X_full = Dropout(rate = 0.2)(X_full)
    #print(X_full.shape)
    
    X_full = Reshape((1, 128))(X_full)
    #print(X_full.shape)
    
    #X_full = TimeDistributed(Dense(5))(X_full)
    #print(X_full.shape)
    
    X_full = LSTM(units = 128, recurrent_dropout = 0.2, return_sequences = True)(X_full)
    #print(X_full.shape)
    X_full = Flatten()(X_full)
